...
const IndexSpareparts = () => {
    const [indexSpareparts, setIndexSpareparts] = useState([]);
    // const URL = "http://localhost:3000/api"

    const getIndexSpareparts = async () => {
        try {
            let temp = await axios({
                url: `http://localhost:3000/api/sparepart`,
                method: "GET",
            })
            console.log(temp.data)
            setIndexSpareparts(temp.data)
        } catch (err) {
            alert(err)
        }
    }
            ...
            <div className="row">
                ...
                <div className="col-12">
                    <table className="table table-bordered border-dark">
                        ...
                        <tbody>
                            {indexSpareparts.length !== 0 ?
                                indexSpareparts.map((product) => {
...
export default IndexSpareparts